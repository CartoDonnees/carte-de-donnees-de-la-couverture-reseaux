...
function createOptionsPlotOptions(formatterType, techno){
  ...
  } else if(formatterType == 'qualité' || formatterType == 'qualité web' || formatterType == "qualité bis"){
...
function graphiqueSitesCumul(texteLeg, inUnite, inTechno) {
  ...
  if (texteLeg == "3500only") options.yAxis = createOptions_Y_Axis_max(1600);
  else if (texteLeg == "Tous5G") options.yAxis = createOptions_Y_Axis_max(10000);
...
function graphiqueQoSAgglos_Voix(inStrate) {
  ...
  //if (territoireSelectionne.key == "metropole") {
    ...
    niveauxCouverture.push({name : 'Appels maintenus pendant 2 minutes et de qualite parfaite', su : {name :'parfait' , suOpCouleur : 'TBC'}});
